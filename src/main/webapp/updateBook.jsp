        ...
        <div class="container-i">
            ...
            <main>
                ...
                <form action="updateBook" method="POST">
                    <div class="book-info">
                        <div class="left-info">
                            <ul>
                                <li><label>Book ID: </label><input type="text" name="book_id" value="${book_update.id}" readonly></li>

                                <li><label>Book Name: </label><input type="text" name="book_name" value="${book_update.name}"></li>
                                <li><label>Price: </label><input type="number" name="price" step="0.01" value="${book_update.price}"><label>$</label></li>

                                <li><label>Sale: </label><input type="number" name="sale" step="0.01" value="${book_update.sale}"><label>%</label></li>
                                <li>
                                ...
                                <li style="width: 200px;"><img style="width: 200px;height: 200px;" id="imagePreview" src="${book_update.img}">
                        ...
                        <div class="right-info">
                            <ul>
                                <li>
                                    ...
                                    <c:set var="options">
                                        <c:out value="Children books,Comic,Textbook,Psychology books,Fables,Novel,Combo" />
                                    </c:set>

                                    <select class="select-box" name="category_name">
                                        <c:forEach var="option" items="${fn:split(options, ',')}">
                                            <option value="${option}" ${option eq category_book ? 'selected' : ''}>${option}</option>
                                        </c:forEach>
                                    </select>
                                </li>
                                <li><label>Description: </label>
                                    <textarea cols="30" rows="3" name="description" style="resize: none">${book_update.description}</textarea>
                                </li>
                                <li><label>Status: </label>
                                    <select class="select-box" name="status">
                                        <option value="Available">Available</option>
                                        <option value="Sold Out" ${'Sold Out' eq status_book ? 'selected' : ''}>Sold Out</option>
                                    </select>
                                </li>
                            </ul>
            ...
            <script>
                ...
                document.querySelector('.btn-add-product button').addEventListener('click', function (event) {
                    ...
                    if (!bookName || isNaN(price) || isNaN(sale) || sale === undefined || category_name === '' || description === '' || description === null || status === '') {
                        Swal.fire({
                            icon: 'error',
